...
public class ModSaplingBlock extends BushBlock implements IGrowable {
    ...
    public void grow(ServerWorld serverWorld, BlockPos pos, BlockState state, Random rand) {
        if (state.get(STAGE) == 0) {
            ...
            if (!ForgeEventFactory.saplingGrowTree(serverWorld, rand, pos))
                return;
            this.tree.get().func_225545_a_(serverWorld, serverWorld.getChunkProvider().getChunkGenerator(), pos, state,
                    rand);
        }
    }
    ...
    protected boolean isValidGround(BlockState state, IBlockReader worldIn, BlockPos pos) {
        ...
        return block == Blocks.GRASS_BLOCK || block == Blocks.DIRT || block == Blocks.COARSE_DIRT || block == Blocks.PODZOL || block == Blocks.FARMLAND || block == BlockInit.TOPSOIL.get() || block == BlockInit.DECAY.get() || block == BlockInit.MOSS.get() || block == BlockInit.PSYCHEDELIC_DIRT.get() || block == BlockInit.LEAFY_DIRT.get();
    }
